...
router.get('/', authenticate, async (req, res, next) => {
  try {
    ...
    const [eq, active, overdue, issues] = await Promise.all([
      pool.query("SELECT COALESCE(SUM(total_quantity),0)::int AS total,COALESCE(SUM(available_quantity),0)::int AS available,COALESCE(SUM(maintenance_quantity),0)::int AS maintenance FROM equipment"),
