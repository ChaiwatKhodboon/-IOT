...
router.post('/', authenticate, adminOnly, async (req, res, next) => {
  ...
  const d = parsed.data;
  try {
    const { rows } = await pool.query('INSERT INTO equipment(code,name,category,description,total_quantity,available_quantity,status) VALUES($1,$2,$3,$4,$5,$5,$6) RETURNING *', [d.code,d.name,d.category,d.description,d.totalQuantity,d.status]);
...
router.put('/:id', authenticate, adminOnly, async (req, res, next) => {
  ...
  const d = parsed.data;
  try {
    const { rows } = await pool.query(`UPDATE equipment SET code=$1,name=$2,category=$3,description=$4,total_quantity=$5,available_quantity=available_quantity+($5-total_quantity),status=$6,updated_at=NOW() WHERE id=$7 AND available_quantity+($5-total_quantity)>=0 RETURNING *`, [d.code,d.name,d.category,d.description,d.totalQuantity,d.status,req.params.id]);
